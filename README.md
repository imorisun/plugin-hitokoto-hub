...
| 统计数据最大保留条数 | 10000 | 浏览记录与点赞记录分别计算，超过各自上限自动删除最旧数据（100-10000）。每次随机获取句子都会产生一条浏览记录，数值过小会导致数据看板很快丢失历史 |
...
| 目标分类 | 未分类 | 生成句子保存到哪个分类，默认为内置的「未分类」；手动或定时生成前必须确定该分类 |
| 是否自动发布 | true | 开启后生成的句子自动发布；关闭则保存为未发布状态，需在后台手动发布 |
...
| 审核通过后自动发布 | true | 开启后审核通过的句子自动发布；关闭则需在后台手动发布 |
